...
def add_servers(req: Any = Body(...)):
    try:
        print('started the add endpoint')
        ...
        for server_name in servers:
            ...
            for sh in servers[server_name]:
                ...
                app.hash_dict[sh].add_server(
                    app.server_list[server_name]["index"], ip[server_name], 8000
                )
                ...
                while True:
                    ...
                    if sh not in new_shardids:
                        mysql_cursor.execute(
                            "SELECT Server_id FROM MapT WHERE Shard_id=%s AND Primary_=1",
                            (sh,),
                        )
                        ...
                        PRIMARY_SERVER = sh_servers[0][0]
                        print('primarys server : ',PRIMARY_SERVER)

                        try:
                            resp = requests.get(
                                f"http://{ip[server_name]}:8000/fetch_log_data",
                                json={
                                    "shards": sh,
                                    "servers": PRIMARY_SERVER,
                                    "own_ip": ip[server_name],
                                },
                            )
                            # resp = requests.get(f"http://{app.server_list[sh_server[0]]['ip']}:8000/copy",json={
                            # "shards": [sh]},timeout=15)
                            # sh_students = resp.json()[sh]
                            print("response added ::", resp.json())
                            print(resp.status_code)
                            break
                        except requests.RequestException as e:
                            print(e)
                            print("something happedened bro check")
                            # print(
                            #     f"couldn't copy {sh} from {sh_server[0]},trying another server"
                            # )
                    else:
                        break
                    ...
                    # requests.post(
                    #     f"http://{app.server_list[server_name]['ip']}:8000/write",
                    #     json={"shard": sh, "curr_idx": 0, "data": sh_students},
                    #     timeout=15,
                    # )
                    # print(
                    #     f"Successfully copied shard:{sh} data from ",
                    #     sh_server[0],
                    #     " to ",
                    #     server_name,
                    # )
        ...
        for shard in new_shards:
            ...
            print('shard got inserted')
        # print(server_name)
        # mysql_cursor.execute("INSERT INTO MapT VALUES(%s,%s,0)", (sh, server_name))
        mysql_conn.commit()
        # print(
        #     f"Successfully inserted shard:{sh} to server:{server_name} mapping into MapT",
        #     flush=True,
        # )
        return {"N": len(app.server_list), "message": message, "status": "success"}
